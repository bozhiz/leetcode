...
public:
    int minPathSum(vector<vector<int>> &grid)
    {
        ...
        return minGrid(grid, m - 1, n - 1);
    }

private:
    int minGrid(vector<vector<int>> &grid, int i, int j)
    {
        int m = grid.size(), n = m ? grid[0].size() : 0;
        ...
        if (i == 0 && j == 0)
            ...
            return grid[i][j] + minGrid(grid, i, j - 1);
        else if (j == 0)
            return grid[i][j] + minGrid(grid, i - 1, j);

        return grid[i][j] + min(minGrid(grid, i, j - 1), minGrid(grid, i - 1, j));
    }
};
